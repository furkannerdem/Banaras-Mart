export const converter = (
  ...
  const operations = {
    jsonParseOperation: {
      ...
      operation: (key, value) =>
        value ? JSON.parse(value) : defaultValues[key] ?? [],
    },
    integerConvertOperation: {
      ...
      operation: (key, value) =>
        isNaN(value) ? defaultValues[key] ?? 0 : parseInt(value),
    },
  ...
  objectKeys.forEach((key) => {
    ...
    for (const operation in operations) {
      ...
      if (operation === "jsonParseOperation" && !toParseInJson) continue;
    ...
    if (newObject[key] !== false && newObject[key] !== 0 && !newObject[key])
      newObject[key] = value;
  });
