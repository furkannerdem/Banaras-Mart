...
const addNewProduct = catchAsyncError(async (req, res, next) => {
  ...
  let productCategory = await categoryModel.findOne({
    name: toCreate.category.toLowerCase(),
  });
  if (!productCategory) {
...
const fetchProducts = catchAsyncError(async (req, res, next) => {
  ...
  if (product_owners) {
    const productOwners = product_owners
      .split(",")
      .map((ownerId) => mongoose.Types.ObjectId.createFromHexString(ownerId));
    // { category: 'sarees,kurti' }
    // $in will get an array containing string separated by ,
  ...
  if (min_discount || min_discount === 0) {
...
const editProduct = catchAsyncError(async (req, res, next) => {
  ...
  await productModel.findByIdAndUpdate(req.params.id, {
    $set: converter(req.body, false, { isReturnable: true }),
  });
...
const editColorAndImages = catchAsyncError(async (req, res, next) => {
  ...
  colors.forEach(({ color, images, mainImage }) => {
    ...
    if (mainImage) findImage[mainImage.public_id] = true;
  ...
  product.colors = product.colors.filter(({ color, images }) => {
    ...
    if (!findColor[color]) {
      toBeDeletedImages = [
        ...toBeDeletedImages,
        ...images.map(({ image: { public_id } }) => public_id),
      ];
    }
  ...
  product.colors = product.colors.map(({ color, images }) => ({
    ...
    images: images.filter(({ image: { public_id } }) => {
      ...
      if (findImage[public_id]) return true;
  ...
  const [newImages, thumbnail] = await uploadAndStore(
    req.files ? req.files : []
  );
...
const deleteProduct = catchAsyncError(async (req, res, next) => {
  ...
  if (!productId) throw new ApiError(400, "provide an id to delete");
...
function integrateImages(colors, newImages) {
  ...
  if (colors.length > 0 && typeof colors[0] === "string") {
    ...
    result = colors.map(({ color, images }, colorIndex) => ({
      ...
      images: [
        ...images,
        ...(newImages[colorIndex] ? newImages[colorIndex] : []),
      ],
    }));
  }
